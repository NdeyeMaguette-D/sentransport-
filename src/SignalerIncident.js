...
function SignalerIncident() {
  ...
  function handleSubmit() {
    ...
    fetch("http://localhost:5000/incidents", {
      ...
      body: JSON.stringify({ ligne, description, lieu: lieu || "Non précisé" }),
    })
      .then(r => {
  ...
  return (
    <div className="signaler">
      ...
      <div className="signaler-form">
        <input type="text" placeholder="Numéro de ligne (ex: 15)"
          value={ligne} onChange={e => setLigne(e.target.value)}
          className="signaler-input" />
        <input type="text" placeholder="Lieu (ex: Colobane)"
          value={lieu} onChange={e => setLieu(e.target.value)}
          className="signaler-input" />
        <textarea placeholder="Description de l'incident..."
          value={description} onChange={e => setDescription(e.target.value)}
          className="signaler-textarea" rows={3} />
        <button onClick={handleSubmit} disabled={enCours} className="signaler-btn">
